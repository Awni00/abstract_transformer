...
1. The experiments section does not explicitly mention parameter counts. The reviewer is concerned about the fairness of the comparisons accounting for model size since a relational attention head contains more parameter than standard self-attention.
...
2. We carried out additional experiments to compare the proposed *DAT* architecture against previously-proposed relational architectures (PrediNet, CoRelNet, Abstractor) on the relational games. We find that *DAT* performs competitively with these narrow-domain architectures despite their strong task-tailored inductive biases. We also add an expanded comparison to the Abstractor.
3. We added new language modeling experiments with models up to 1.3B parameters, trained for 10B tokens of the Fineweb dataset. We find the *DAT* model outperforms the Transformer baseline at multiple model scales, suggesting that the model has learned to use the relational computational mechanisms in language processing. We carried out a visualization and analysis of the learned relational attention heads, which will be added to the paper. This forms a challenging large-scale evaluation of the applicability of relational computational mechanisms to important complex real-world tasks.

---

**Controlling for model capacity/parameter count.**

Your point about controlling for model capacity is well-taken, and we entirely agree is important. Please see the uploaded 1-page pdf for updated experimental results which carefully account for the effect of model capacity. For example, note that in the relational games experiments the *DAT* model is 421K parameters and is compared to Transformers with 386K parameters (same $d_{\mathrm{model}}$) and 481K parameters (increased $d_{\mathrm{model}}$). We also compare against a Transformer with a larger number of heads. Similarly, for the language modeling experiments, note that we compare a 1.31B-parameter Transformer to a 1.27B-parameter *DAT*. A similar same applies for the remaining experiments.
...
Please see the global response for an explanation of how our suite of experiments is designed to evaluate the empirical claims of our paper.

*Additional relational games baselines.* We have added several relational architectures baselines (PrediNet, CoRelNet, Abstractor) to the relational games experiment. We refer to [[[Figure 1]]] in the attached 1-page pdf. We find that *DAT* performs competitively with the relational architectures, all of which outperform the Transformer. This is despite the fact that these architectures have a narrow-domain of applicability and strong inductive biases designed specifically for benchmarks like this. By contrast, *DAT* is a much more general modeling framework which can be applied to a wide range of task paradigms and data modalities (as demonstrated by the remaining experiments).
