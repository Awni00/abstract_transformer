...
2. Sequence-to-sequence mathematical problem-solving. This showcases applicability to a seq2seq task paradigm via an encoder-decoder architecture, and evaluates the architecture on a symbolic processing task, demonstrating improvements over standard Transformers.
...
**Summary of major additions to submission, addressing reviewer comments**

We have added several additional experiments since the original submission, which we hope address reviewer comments.

- *Large-scale language modeling experiments*. We have carried out new large-scale language modeling experiments on the Fineweb dataset, with model sizes up to **1.3B parameters** trained for **10B tokens**. Fineweb is an open source large-scale language modeling dataset consisting of cleaned deduplicated text from CommonCrawl. We find that the *DAT* model outperforms Transformers on language modeling at multiple scales, achieving smaller perplexity. We also performed a visualization of the learned relational attention heads, finding interpretable patterns that correspond to meaningful semantic relations.
- *CoRelNet, PrediNet, and Abstractor baselines have been added to the relational games experiments.* This serves to demonstrate the competitiveness of *DAT* against relational architectures with strong inductive biases, even on synthetic relational tasks.
- *Expanded experiments on the mathematical problem-solving experiments, at multiple depths and model scales.* We find that *DAT* outperforms Transformers on this symbolic processing task consistently across multiple model scales.
- *Additional baselines to rigorously control for effect of model size and hyperparameters in comparisons*. We added more Transformer baselines to each experiment with increased model dimension and an increased number of relational heads to further validate that the superior performance of *DAT* can be attributed to the new relational computational mechanisms rather than model size of architectural hyperparameters.

In addition, we have expanded discussion on architecture design, relational inductive biases, relational attention, and role of symbols.  We hope that this addresses each of the criticisms and concerns that were raised.