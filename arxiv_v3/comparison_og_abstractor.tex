...
In RA, the symbols maintain the role of identifying the sender. But they are now explicitly attached to a separately parameterized relation vector.
...
\Cref{fig:tinystories_ratype_ablation} depicts the validation loss curves on a small-scale language modeling experiment based on the Tiny Stories dataset~\citep{eldanTinyStoriesHowSmall2023}, comparing standard \textit{DAT} against a version with RCA heads. Here, we find that our relational attention heads yield better-performing models, with the RCA-head variant of \textit{DAT} performing no better than a standard Transformer with a matching total number of heads.
